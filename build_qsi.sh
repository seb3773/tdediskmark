...
# ==============================================================================
# tdeDiskMark - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " tdeDiskMark - Q4OS .qsi Package Builder"
echo "=================================================="
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/build/tdediskmark_*.deb "$SCRIPT_DIR"/tdediskmark_*.deb "$SCRIPT_DIR"/build/*.deb "$SCRIPT_DIR"/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "tdediskmark")
...
APPNAME_DESC="tdeDiskMark Storage Benchmark"
APP_ICON="tdediskmark"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
